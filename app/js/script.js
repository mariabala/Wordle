...
for (let i = 0; i < keys.length; i++) {
  keys[i].onclick = ({ target }) => {
    const lett = target.getAttribute("id");
    if (
      lett.length === 1 &&
      pattern.test(lett) &&
      ...
      //let === "enter"
      currentGuess.dataset.letters.length == 5 &&
      allWords.includes(currentGuess.dataset.letters)
    ) {
      submitGuess();
    }
  };
...
const submitGuess = () => {
  revealingTile = true;
  for (let i = 0; i < 5; i++) {
    let s = currentGuess.dataset.letters.charAt(i);
    //console.log("hsgwvdgasvd");
    console.log(s);
    //let keyLetter = s.dataset.letters;
    setTimeout(() => {
      let a = checkLetter(i);
...
const flipTile = (tileNum, state) => {
  ...
  //const tiles = document.querySelector("#guess" + currentGuessCount).childNodes;
  //console.log(tiles);
  ...
  //   console.log("hdjfjdjddj");
  //   let dataLetter = currentGuess.dataset.letters;
  //   setTimeout(() => {
  //     if (dataLetter === solution[index]) {
